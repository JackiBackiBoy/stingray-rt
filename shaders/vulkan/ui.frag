...
void main() {
    if (fsInUIType == UI_TYPE_RECTANGLE) {
        fsOutColor = vec4(texture(sampler2D(g_Textures[fsInTexIndex], g_Samplers[0]), fsInTexCoord).rgb, 1.0f);
    }
    else if (fsInUIType == UI_TYPE_TEXT) {
        fsOutColor = vec4(1.0f, 1.0f, 1.0f, texture(sampler2D(g_Textures[fsInTexIndex], g_Samplers[0]), fsInTexCoord).r);
    }
}