...
#ifdef SR_VULKAN
	#define SR_VK_BINDING(binding, set) [[vk::binding(binding, set)]]
	#define SR_VK_PUSH_CONSTANT [[vk::push_constant]]
#else
	#define SR_VK_BINDING(binding, set)
	#define SR_VK_PUSH_CONSTANT
#endif
...
#define SR_PUSH_CONSTANT(type, name) SR_VK_PUSH_CONSTANT cbuffer PushConstant##name : register(b0, space0) { type name; }
