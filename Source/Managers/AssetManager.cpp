...
namespace SR {
	GraphicsDevice* g_GfxDevice = nullptr;
	...
	namespace AssetManager {
		...
		void initialize(GraphicsDevice& gfxDevice, MaterialManager& materialManager) {
		...
		std::unique_ptr<Model> create_sphere(float radius, int latitudes, int longitudes, const Material* material) {
			...
			const float latAngStep = glm::pi<float>() / static_cast<float>(latitudes);
			const float lonAngStep = glm::two_pi<float>() / static_cast<float>(longitudes);
			float latAng;
			float lonAng;
			float rCosLatAng;
			...
			for (int lon = 0; lon <= longitudes; ++lon) {
				lonAng = static_cast<float>(lon) * lonAngStep;

				for (int lat = 0; lat <= latitudes; ++lat) {
					latAng = glm::half_pi<float>() - static_cast<float>(lat) * latAngStep;
					...
					vertex.texCoord.x = static_cast<float>(lon) / static_cast<float>(longitudes);
					vertex.texCoord.y = static_cast<float>(lat) / static_cast<float>(latitudes);
			...
			uint32_t k1, k2;
			for (int lon = 0; lon < longitudes; ++lon) {
				k1 = static_cast<uint32_t>(lon * (latitudes + 1));
				k2 = static_cast<uint32_t>(k1 + latitudes + 1);
		...
		Font* load_font_from_file(const std::string& path, int ptSize) {
			...
			unsigned int maxDim = 1U << (unsigned int)(1 + ceilf(log2f((float)std::max(atlasWidth, atlasHeight) / ceilf(sqrtf((float)numGlyphs)))));
			atlasWidth = maxDim;
			atlasHeight = maxDim;
			...
			for (unsigned char c = 32; c < 127; ++c) {
				...
				const float glyphCoordTop = static_cast<float>(atlasOffsetY) / static_cast<float>(atlasHeight);
				const float glyphCoordLeft = static_cast<float>(atlasOffsetX) / static_cast<float>(atlasWidth);
				const float glyphCoordBottom = static_cast<float>(atlasOffsetY + bmp->rows) / static_cast<float>(atlasHeight);
				const float glyphCoordRight = static_cast<float>(atlasOffsetX + bmp->width) / static_cast<float>(atlasWidth);
