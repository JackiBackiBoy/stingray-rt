...
#include "Graphics/Vulkan/GraphicsTypesVulkan.h"
...
namespace SR {
	static constexpr std::array<const char*, 2> REQUIRED_INSTANCE_EXTENSIONS = {{
		VK_KHR_SURFACE_EXTENSION_NAME,
		VK_KHR_WIN32_SURFACE_EXTENSION_NAME
	}};

	// GraphicsDeviceVulkan Implementation
	struct GraphicsDeviceVulkan::Impl {
		Impl(Window& window);
		~Impl();
		...
		DescriptorHeap m_UBODescriptorHeap = DescriptorHeap(GraphicsDevice::MAX_UBO_DESCRIPTORS);
		DescriptorHeap m_TextureDescriptorHeap = DescriptorHeap(GraphicsDevice::MAX_TEXTURE_DESCRIPTORS);
		DescriptorHeap m_StorageBufferDescriptorHeap = DescriptorHeap(GraphicsDevice::MAX_STORAGE_BUFFERS);
		DescriptorHeap m_RWTextureDescriptorHeap = DescriptorHeap(GraphicsDevice::MAX_RW_TEXTURE_DESCRIPTORS);
	...
	void GraphicsDeviceVulkan::Impl::Descriptor::init_ubo(Impl* impl, VkBuffer buffer) {
	...
	void GraphicsDeviceVulkan::Impl::Descriptor::init_texture(Impl* impl, VkImageView imageView, VkImageLayout layout) {
	...
	void GraphicsDeviceVulkan::Impl::Descriptor::init_rw_texture(Impl* impl, VkImageView imageView, VkImageLayout layout) {
		const VkDescriptorImageInfo descriptorImageInfo = {
			.sampler = nullptr,
			.imageView = imageView,
			.imageLayout = layout
	...
	void GraphicsDeviceVulkan::Impl::Descriptor::init_storage_buffer(Impl* impl, VkBuffer buffer) {
		const VkDescriptorBufferInfo descriptorBufferInfo = {
			.buffer = buffer,
			.offset = 0,
			.range = VK_WHOLE_SIZE
	...
	GraphicsDeviceVulkan::Impl::Impl(Window& window) : m_Window(window) {
		create_instance();
	...
	void GraphicsDeviceVulkan::Impl::create_instance() {
		...
		// NOTE: Win32 only
		// TODO: Although we will only use Vulkan for Win32, still worth to make cleaner
		...
		size_t numFoundReqExtensions = 0;
		for (const auto& extension : extensionProperties) {
			std::cout << '\t' << extension.extensionName;

			for (const char* reqExtensionName : REQUIRED_INSTANCE_EXTENSIONS) {
				if (strcmp(extension.extensionName, reqExtensionName) == 0) {
					std::cout << " (REQUIRED)";
					++numFoundReqExtensions;
					break;
				}
			}
		...
		assert(numFoundReqExtensions == REQUIRED_INSTANCE_EXTENSIONS.size());
	...
	void GraphicsDeviceVulkan::Impl::create_debug_messenger() {
		if (!m_EnableValidationLayers) {
			return;
		}
	...
	void GraphicsDeviceVulkan::Impl::create_surface() {
		const VkWin32SurfaceCreateInfoKHR surfaceInfo = {
			.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR,
			.hinstance = GetModuleHandle(nullptr),
			.hwnd = (HWND)m_Window.get_internal_object()
		};
	...
	void GraphicsDeviceVulkan::Impl::pick_physical_device() {
	...
	void GraphicsDeviceVulkan::Impl::create_device() {
	...
	void GraphicsDeviceVulkan::Impl::create_destruction_handler() {
		m_DestructionHandler.device = m_Device;
		m_DestructionHandler.instance = m_Instance;
	}

	void GraphicsDeviceVulkan::Impl::create_command_pool() {
	...
	void GraphicsDeviceVulkan::Impl::create_sync_objects() {
	...
	void GraphicsDeviceVulkan::Impl::create_descriptors() {
		// Descriptor pool
	...
	bool GraphicsDeviceVulkan::Impl::check_validation_layers() {
		uint32_t layerCount;
	...
	bool GraphicsDeviceVulkan::Impl::is_device_suitable(VkPhysicalDevice device) {
	...
	VKAPI_ATTR VkBool32 VKAPI_CALL GraphicsDeviceVulkan::Impl::debug_callback(
		VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
		VkDebugUtilsMessageTypeFlagsEXT messageType,
		const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData,
		void* pUserData)
	...
	VkResult GraphicsDeviceVulkan::Impl::create_debug_utils_messenger(
		VkInstance instance,
		const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo,
		const VkAllocationCallbacks* pAllocator,
		VkDebugUtilsMessengerEXT* pDebugMessenger)
	...
	void GraphicsDeviceVulkan::Impl::destroy_debug_utils_messenger(
		VkInstance instance,
		VkDebugUtilsMessengerEXT debugMessenger,
		const VkAllocationCallbacks* pAllocator)
	{
	...
	void GraphicsDeviceVulkan::Impl::populate_debug_messenger_create_info(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
		createInfo = {};
	...
	void GraphicsDeviceVulkan::Impl::create_internal_swapchain(SwapChain_Vulkan* internalState) {
		...
		if (supportInfo.capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
			...
			const int width = m_Window.get_client_width();
			const int height = m_Window.get_client_height();
	...
	GraphicsDeviceVulkan::Impl::QueueFamilyIndices GraphicsDeviceVulkan::Impl::find_queue_families(VkPhysicalDevice device) {
	...
	uint32_t GraphicsDeviceVulkan::Impl::find_memory_type(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
	...
	bool GraphicsDeviceVulkan::Impl::check_device_extension_support(VkPhysicalDevice device) {
	...
	GraphicsDeviceVulkan::Impl::SwapChainSupportInfo GraphicsDeviceVulkan::Impl::query_swapchain_support(VkPhysicalDevice device) {
	...
	// GraphicsDeviceVulkan Interface
	GraphicsDeviceVulkan::GraphicsDeviceVulkan(Window& window) : GraphicsDevice(window) {
	...
	void GraphicsDeviceVulkan::create_swapchain(const SwapChainInfo& info, SwapChain& swapChain) {
	...
	void GraphicsDeviceVulkan::create_pipeline(const PipelineInfo& info, Pipeline& pipeline) {
	...
	void GraphicsDeviceVulkan::create_buffer(const BufferInfo& info, Buffer& buffer, const void* data) {
		...
		if (bufferInfo.usage & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT) {
			...
			bdaInfo.buffer = internalState->buffer;
			internalState->address = vkGetBufferDeviceAddress(m_Impl->m_Device, &bdaInfo);
		}
	...
	void GraphicsDeviceVulkan::create_shader(ShaderStage stage, const std::string& path, Shader& shader) {
	...
	void GraphicsDeviceVulkan::create_texture(const TextureInfo& info, Texture& texture, const SubresourceData* data) {
	...
	void GraphicsDeviceVulkan::create_sampler(const SamplerInfo& info, Sampler& sampler) {
	...
	// -------------------------------- Ray Tracing --------------------------------
	void GraphicsDeviceVulkan::create_rtas(const RTASInfo& rtasInfo, RTAS& rtas) {
	...
	void GraphicsDeviceVulkan::create_rt_instance_buffer(Buffer& buffer, uint32_t numBLASes) {
	...
	void GraphicsDeviceVulkan::create_rt_pipeline(const RTPipelineInfo& info, RTPipeline& pipeline) {
	...
	void GraphicsDeviceVulkan::create_shader_binding_table(const RTPipeline& pipeline, uint32_t groupID, ShaderBindingTable& sbt) {
	...
	void GraphicsDeviceVulkan::write_blas_instance(const RTTLAS::BLASInstance& instance, void* dst) {
	...
	void GraphicsDeviceVulkan::build_rtas(RTAS& rtas, const CommandList& cmdList) {
	...
	void GraphicsDeviceVulkan::bind_rt_pipeline(const RTPipeline& pipeline, const CommandList& cmdList) {
	...
	void GraphicsDeviceVulkan::push_rt_constants(const void* data, uint32_t size, const RTPipeline& pipeline, const CommandList& cmdList) {
		assert(data != nullptr);
		assert(size <= 128);
	...
	void GraphicsDeviceVulkan::dispatch_rays(const DispatchRaysInfo& info, const CommandList& cmdList) {
		auto internalCmdList = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::bind_pipeline(const Pipeline& pipeline, const CommandList& cmdList) {
		auto internalPipeline = to_internal(pipeline);
		auto internalCmdList = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::bind_viewport(const Viewport& viewport, const CommandList& cmdList) {
		auto internalCmdList = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::bind_uniform_buffer(const Buffer& uniformBuffer, uint32_t slot) {

	}

	void GraphicsDeviceVulkan::bind_vertex_buffer(const Buffer& vertexBuffer, const CommandList& cmdList) {
	...
	void GraphicsDeviceVulkan::bind_index_buffer(const Buffer& indexBuffer, const CommandList& cmdList) {
	...
	void GraphicsDeviceVulkan::push_constants(const void* data, uint32_t size, const CommandList& cmdList) {
		assert(data != nullptr);
		assert(size <= 128); 
	...
	void GraphicsDeviceVulkan::barrier(const GPUBarrier& barrier, const CommandList& cmdList) {
		auto internalCmdList = to_internal(cmdList);
		...
		case GPUBarrier::Type::IMAGE:
			{
				...
				const bool isDepth = is_depth_format(textureInfo.format);
	...
	CommandList GraphicsDeviceVulkan::begin_command_list(QueueType queue) {
	...
	void GraphicsDeviceVulkan::begin_render_pass(const SwapChain& swapChain, const PassInfo& passInfo, const CommandList& cmdList, bool clear) {
	...
	void GraphicsDeviceVulkan::begin_render_pass(const PassInfo& passInfo, const CommandList& cmdList) {
		auto internalCmdList = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::end_render_pass(const SwapChain& swapChain, const CommandList& cmdList) {
		auto internalSwapChain = to_internal(swapChain);
		auto internalCmdList = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::end_render_pass(const CommandList& cmdList) {
		auto internalCmdList = to_internal(cmdList);

		vkCmdEndRendering(internalCmdList->commandBuffers[m_CurrentFrame]);
	}

	void GraphicsDeviceVulkan::submit_command_lists(const SwapChain& swapChain) {
		auto internalSwapChain = to_internal(swapChain);
	...
	void GraphicsDeviceVulkan::draw(uint32_t vertexCount, uint32_t startVertex, const CommandList& cmdList) {
	...
	void GraphicsDeviceVulkan::draw_indexed(uint32_t indexCount, uint32_t startIndex, uint32_t baseVertex, const CommandList& cmdList) {
		auto internalCommandBuffer = to_internal(cmdList);
	...
	void GraphicsDeviceVulkan::draw_instanced(uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertex, uint32_t startInstance, const CommandList& cmdList) {
		auto internalCommandBuffer = to_internal(cmdList);
	...
	uint32_t GraphicsDeviceVulkan::get_descriptor_index(const Resource& resource, SubresourceType type) {
	...
	uint64_t GraphicsDeviceVulkan::get_bda(const Buffer& buffer) {
		return m_Impl->to_internal(buffer)->address;
	}

	void GraphicsDeviceVulkan::wait_for_gpu() {
